...
class BinarySearchTree:
    ...
    def search(self, data):
        ...
        while p:
            if data < p.val:
                p = p.left
            else:  # 二叉树存在多个值时候，将第二个值假装大于，放到右边的节点，所以判断data是否存在，还需要遍历右边的树
                if data == p.val:
                    res.append(p)
                p = p.right
        return res
    ...
    def delete(self, data):
        ...
        for p in del_list:
            ...
            if p.parent is None and p != self.root:
                continue
            else:
                self._del(p)
    ...
    def _del(self, node):
        ...
        # 3
            ...
            if node.val != min_node.val:
                ...
                self._del(min_node)
            # 右子树的最小值节点与被删除节点的值相等，再次删除原节点,递归删除相同值的节点
            else:
                self._del(min_node)
                self._del(node)
    ...
    def _in_order(self, node):
        ...
        ret = []
        p = node
        ret.extend(self._in_order(p.left))
        ret.append(p.val)
        ret.extend(self._in_order(p.right))
        # print("ret",ret)
        return ret
